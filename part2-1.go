...
// // Faulty coordinator. Breaks after awhile.
...
// // 1. if rejected: stop the election 
// // 2. else (timeout): client is elected
...
// 			fmt.Println("Received", caller_id, "rejected as coordinator.")
...
// // Simulate msg exchange with faulty coordinator and listen to election calls by other clients 
...
// 		go func() { for { node_chans[4] <- id } }()
		
// 		// receive ACK messages 
// 		loop := true
// 		for loop {
// 			count++
// 			fmt.Println("Node", id, "sent message to Node 5")
// 			select {
// 			case <-node_chans[id-1]:
// 				fmt.Println("Node", id, "received ACK from Node 5")
// 			case <-time.After(1 * time.Second):
// 				// Call for election
// 				fmt.Println("(Timeout) No response from Node 5. Node", id, "Calling election...")
// 				election_chan <- id
